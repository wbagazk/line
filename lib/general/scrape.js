...
async function checkIP() {
  try {
    ...
    process.exit(1); // Menghentikan sistem secara otomatis.
  }
}
...
async function verifyPhoneNumber(Line) {
  ...
  while (!isAuthorized) {
    ...
    phoneNumber = await getInput(chalk.yellow.bold('Nomor: '));
    ...
    // Periksa apakah nomor berada di blacklist
    if (blacklist.includes(phoneNumber.trim())) {
      console.log(chalk.red.bold('Nomor ini telah di-*banned*! Akses ditolak.'));
      continue; // Ulangi loop untuk memasukkan nomor baru
    }

    // Periksa apakah nomor berada di daftar diizinkan
