...
const jadibot = async (Line, m, from) => {
  if (Object.keys(client).includes(from)) {
    return Line.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await Line.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../../Line')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(Line, m, from) {
  if (!Object.keys(client).includes(from)) {
    return Line.sendMessage(from, {
...
async function listjadibot(Line, m) {
