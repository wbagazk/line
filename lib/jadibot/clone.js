...
const {
  ...
  writeExifImgAV,
  writeExifVid
} = require('../general/exif')
const {
  getBuffer,
  sleep,
  smsg
} = require('../general/myfunc')
...
const jadibot = async (vioo, m, from) => {
  if (Object.keys(client).includes(from)) {
    return vioo.sendMessage(from, {
  ...
  const {
    state,
    saveCreds
  } = await useMultiFileAuthState(`./data/clone/${m.sender.split("@")[0]}`)
  try {
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await client[from].sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../../vioo')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(vioo, m, from) {
  if (!Object.keys(client).includes(from)) {
    return vioo.sendMessage(from, {
  ...
  delete client[from]
  fs.unlinkSync(`./data/clone/${from.split("@")[0]}`)
}

async function listjadibot(vioo, m) {
