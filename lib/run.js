...
const { default: makeWASocket, useMultiFileAuthState, DisconnectReason, generateForwardMessageContent, generateWAMessageFromContent, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto, Browsers } = require('@whiskeysockets/baileys')
...
console.log(chalk.yellow.bold(figlet.textSync('Line', {
font: 'Standard',
...
if (!mek.message || processedMessages.has(mek.key.id)) return
processedMessages.add(mek.key.id)
if (mek.key.remoteJid === 'status@broadcast') return
mek.message = mek.message.ephemeralMessage?.message || mek.message
if (mek.key.fromMe) {
const isManualMessage = mek.message.conversation && mek.message.conversation.trim() !== ''
if (!isManualMessage) return
}
...
avatarUrl = 'https://files.catbox.moe/nej72m.jpg';
}
if (anu.action === 'add') {
